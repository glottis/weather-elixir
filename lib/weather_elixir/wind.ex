defmodule WeatherElixir.Wind do
  ...
  def calc_wind_speed() do
    ...
    state = get()
    count = state[:count]
    interval = @wind_interval_ms / 1000

    case count > 0 do
      true ->
	count_time = (count * @wind_close_second) / interval
        adjusted_speed = (count_time * @factor) / 3.6 |> Float.round(1)

        speed = if adjusted_speed > 30, do: 0, else: adjusted_speed
